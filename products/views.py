# Code for views.py from Code Institute's Boutique Ado walk-through
...
def all_products(request):
    ...
    if request.GET:
        ...
        if 'q' in request.GET:
            query = request.GET['q']
            if not query:
                messages.error(request, "Search criteria not entered!")
...
def add_product(request):
    ...
    if not request.user.is_superuser:
        ...
        return redirect(reverse('products_home'))
...
def edit_product(request, product_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
           ...
           messages.error(request, 'Product update Failed! Please check form details.')
    else:
