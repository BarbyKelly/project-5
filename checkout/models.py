...
class Order(models.Model):
    ...
    def update_total(self):
        ...
        if self.order_total < settings.FREE_POSTAGE_THRESHOLD:
            sdp = settings.STANDARD_POSTAGE_PERCENTAGE
            self.postage_cost = self.order_total * sdp / 100
        else:
            self.postage_cost = 0
...
class OrderLineItem(models.Model):
    ...
    product_size = models.CharField(
        max_length=10, null=True, blank=True) # 13x10cm, 10x13cm, 18x13cm, 13x18cm, 45x30cm, 30x45cm
